...
\section{Upper control on unqualified faces}

The former assertion that every unqualified face contributes nothing is
false for signed perturbations. On $[0,1]^2$, take $D_0=x^6+y^6$ and
$R=-x^2+xy^2$. All face initial forms are non-positive, yet
\[
 -D_0+sR=\frac{s^3}{432}
 -(y^2-s/6)^2(y^2+s/12)-s(x-y^2/2)^2-x^6.
\]
Evaluating at $y=\sqrt{s/6}$, $x=s/12$ gives
\[
 \frac{s^3}{432}-\frac{s^6}{12^6}\le\Delta(s)\le\frac{s^3}{432}.
\]
Uniform convergence to a non-positive initial form does not preserve its
sign near its zero set. See \texttt{MATHEMATICAL\_AUDIT.md} for the proof,
a family of counterexamples, and the implemented sufficient certificates.

\begin{lemma}[safe exclusions]\label{lem:inadmissible}
An inactive face gives no improvement. A face on which
$R_+(c):=\max(R(c),0)\le K D_0(c)$ locally also gives no improvement for
small $s$. This includes faces whose surviving terms all have degree at
least one, and faces on which $R\le0$ locally.
\end{lemma}
\begin{proof}
The compact closed section $D_0(z)=1$ and Hypothesis~\ref{hyp:base}
give $D(c)\ge aD_0(c)$ near the origin for some $a>0$. Hence
$-D+sR\le(-a+sK)D_0\le0$ when $sK<a$. Terms of degree at least one
are bounded by a constant times $D_0$ on the same normalized section.
\end{proof}

\begin{hypothesis}[uniform positive-gain envelope]\label{hyp:envelope}
The independently qualified faces have a minimum $q^\star\in(0,1)$,
and there is a finite $K$ such that, near the vertex,
\[
 R_+(c)\le K D_0(c)^{q^\star}.
\]
\end{hypothesis}
This is automatic for nonnegative combined coefficients, and when the
full cone's lowest layer has a positive witness. Other signed polynomials
need separate control near zeros of non-positive initial forms.

\section{The facewise balance}\label{sec:balance}

Fix an admissible $S$ and the closed cross-section
$Z=\{z\in C_S:D_0(z)=1\}$, compact by positive base coefficients.
It includes the boundary; its relative-interior portion need not be compact. Write
...
\[
  \sup_{c\in\relint(C_S),\ c\text{ near }0}J_s=\Theta\bigl(s^{1/(1-q_S)}\bigr),
  \qquad
...
Assume Hypotheses~\ref{hyp:iso}--\ref{hyp:envelope} and that at least one face
...
\begin{proof}
Lemma~\ref{lem:isolation} localises the maximum. Put $t=D_0(c)$.
Hypotheses~\ref{hyp:base} and~\ref{hyp:envelope} give
$-D+sR\le-at+sKt^{q^\star}$ for some $a>0$. The supremum of this scalar
expression is $O(s^{1/(1-q^\star)})$. A fixed positive witness on a
qualified minimizing face gives the matching lower bound by
...
Hypotheses~\ref{hyp:iso}--\ref{hyp:envelope}, the asymptotic exponent is
...
Failure of these tests does not imply absence of positive improvement.
Lemma~\ref{lem:inadmissible} gives safe exclusions; the additional uniform
envelope in Hypothesis~\ref{hyp:envelope} controls the full supremum.
...
inadmissible face, omitting it is safe only after verifying the upper envelope.
Curved approaches near its zeros can otherwise expose a missed mechanism.
...
\subsection*{1. The tangent cone organises the singularity}

Near a simple vertex, the tangent cone carries the local optimization
problem. Its full-dimensional face must be included: a coupled perturbation
can attain its optimum in the relative interior of the full cone.
...
Projection onto a proper face need not preserve perturbative gain. For
$D_0=x^4+y^4$ and $R=xy$, every proper face has zero gain while the
interior has positive gain for small $s$. The face partition
\eqref{eq:partition} includes the full cone and does not assert boundary
optimality. Signed perturbations can also require curved approaches beyond
the first initial forms, as the counterexample above shows.
...
smaller exponent and, for small $s$, a strictly larger improvement. Comparing
...
\begin{enumerate}[nosep]
  ...
  \item \emph{Selection}: admissible faces are ranked by weighted degree
        with upper control (Definition~\ref{def:adm}, Hypothesis~\ref{hyp:envelope}).
...
\begin{itemize}
  ...
  \item \textbf{explains the mechanism} --- the tangent cone organises the
        singularity; under the uniform envelope
        the lowest admissible weight dominates
  ...
  \item \textbf{identifies candidate active constraints} --- under the theorem's
        hypotheses, a measured exponent gives the effective weight
        $q^\star=1-1/\gamma$. Multiple faces may realize it; exact optimizer
        support requires further reduced and possibly subleading analysis;
