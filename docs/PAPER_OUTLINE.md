...
- 3-block failure mode when $\varepsilon \gt  0.25$
- **Non-quadratic:** `experiments/nonlinear_results.json` — `face_bowl` interaction shows grid max $\gt $ vertex max (Theorem 1 requires hypotheses)
